...
void
filters_copy(image_t *dst, const image_t *src,
  ...
  for (int xx = x; xx < x + width; xx++)
    {
      for (int yy = y; yy < y + height; yy++)
        *image_get_pixel(dst, xx, yy) = *image_get_pixel(src, xx, yy);
    }
}

void
filters_apply_gamma(image_t *image,
                    const int x, const int y,
                    const int width, const int height)
{
  for (int xx = x; xx < x + width; xx++)
    {
      for (int yy = y; yy < y + height; yy++)
        {
          /* Get the i-th pixel */
          uint32_t pixel = *image_get_pixel(image, xx, yy);
          ...
          *image_get_pixel(image, xx, yy) = result;
        }
    }
}

#define RGBA_unpack_new(dst, src, f) \
(dst).x = (float)(((src) >> 24) & 0xff) * f; \
(dst).y = (float)(((src) >> 16) & 0xff) * f; \
(dst).z = (float)(((src) >> 8) & 0xff) * f; \
(dst).w = (float)((src) & 0xff) * f;

#define divfloat 1/255.;

void
filters_brightness(image_t *image,
                   const int x, const int y,
                   const int width, const int height,
                   const float mult)
{
      ...
      for (int yy = y; yy < y + height; yy++)
        {
           ...
           */
          rgba_t color;
          RGBA_unpack_new(color, *image_get_pixel(image, xx, yy), divfloat);

          RGBA_mults(color, color, mult);
          RGBA_pack(*image_get_pixel(image, xx, yy), color);
        }
    }
}
...
void
filters_selective_grayscale(image_t *image,
                            const int x, const int y,
                            const int width, const int height,
                            const int hue, const int spread)
{
      ...
      for (int yy = y; yy < y + height; yy++)
        {
           ...
           */
          float intensity =
              color.w * ( 0.2126f * color.x + 0.7152f * color.y +
                          0.0722 * color.z);
...
void
filters_gaussian_blur(image_t *dst, const image_t *src, const float sigma)
{
              ...
              for (int i = 1; i <= spread; i++)
                {
                   ...
                   */
                  float coeff = exp(-.5f * (float)i * (float)i / (sigma * sigma));
                  ...
                  RGBA_unpack(temp, *image_get_pixel(src_img,
                                                     clamped_x, clamped_y));
