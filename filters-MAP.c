  ...
  for (int xx = x; xx < x + width; xx++)
    {
      for (int yy = y; yy < y + height; yy++)
        *image_get_pixel(dst, yy, xx) = *image_get_pixel(src, yy, xx);
    }
}
  ...
  for (int xx = x; xx < x + width; xx++)
    {
      for (int yy = y; yy < y + height; yy++)
        {
          /* Get the i-th pixel */
          uint32_t pixel = *image_get_pixel(image, yy, xx);
          ...
          *image_get_pixel(image, yy, xx) = result;
        }
    }
}
      ...
      for (int yy = y; yy < y + height; yy++)
        {
           ...
           */
          rgba_t color;
          RGBA_unpack(color, *image_get_pixel(image, yy, xx));
          RGBA_mults(color, color, mult);
          RGBA_pack(*image_get_pixel(image, yy, xx), color);
        }
    }
}
